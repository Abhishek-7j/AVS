"""
Surface Intel Fusion — correlates DNS identity, HTTP banners, and TLS material
in parallel. This is the “know the target before judging risk” layer.
"""
...
def _http_fingerprint(host: str, port: int, use_tls: bool, server_hostname: str | None) -> dict[str, Any]:
    ...
    try:
        ...
        data = b""
        while len(data) < 98304:
            chunk = sock.recv(8192)
            if not chunk:
                break
            data += chunk
            if b"\r\n\r\n" in data and len(data) > 8192:
                break
        ...
        lines = head.split(b"\r\n")
        if lines:
            rec["status_line"] = lines[0].decode(errors="replace").strip()
        headers: dict[str, str] = {}
        for line in lines[1:]:
            if b":" in line:
                k, v = line.split(b":", 1)
                key = k.decode(errors="replace").strip().lower()
                headers[key] = v.decode(errors="replace").strip()
        keep = (
            "server",
            "x-powered-by",
            "set-cookie",
...
def gather_intel(query: str, resolved_ip: str, scan_rows: list[tuple]) -> TargetIntel:
    ...
    def run_http(args: tuple[int, bool]) -> dict[str, Any]:
        p, tls = args
        return _http_fingerprint(ipv4, p, tls, sni)

    with ThreadPoolExecutor(max_workers=min(16, max(1, len(jobs)))) as ex:
        futs = {ex.submit(run_http, j): j for j in jobs}
